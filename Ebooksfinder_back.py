...
class EPUB_books_finder():
	...
	def copy2zipdir(self,zip_dir):
		...
		shutil.copy(self.epub_name, new_name)           #注意:windows下delimiter是\
	...
	def rename_epub2zip(self,epub_name):
		...
		if epub_name.endswith('.epub') or epub_name.endswith('.mobi'):
			shutil.move(epub_name,zip_name)
			return zip_name
		else:
			return False
	...
	def unzip_file(self,zip_name):
		...
		if not zip_name:
			sys.exit(-1)
		try:
			with zipfile.ZipFile(zip_name) as zipObj:
				zipObj.extractall()
			success = True
		...
		for parent,dirnames,filenames in os.walk(zipdir):
			...
			for filename in filenames:    
				if self.isxhtml(filename):
					html_file = os.path.join(parent,filename)
